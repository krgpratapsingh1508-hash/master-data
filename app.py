        ...
        # ----------------------------------------------------------------------
            ...
            if ordered_db_display.empty:
                ...
                if st.session_state.admin_lock_state:
                    ...
                    # ======================================================================
                    # 🧬 न्यू सब-सिस्टम: Subject ↔ Degree ↔ Duration Auto-Fix Engine (Admin Control)
                    #    नियम 1: अगर 'Subject' खाली है, तो उसमें 'Degree' की वैल्यू भर जाए
                    #    नियम 2: अगर 'Degree' में ब्रैकेट में कोई संख्या लिखी है (जैसे "B.Sc. (3)"),
                    #            तो वो संख्या 'Duration' में चली जाए और Degree से ब्रैकेट + उसके अंदर का हिस्सा हट जाए
                    # ======================================================================
                    if not live_db.empty and "Degree" in live_db.columns:
                        st.markdown("---")
                        st.subheader("🧬 Subject ↔ Degree ↔ Duration Auto-Fix Engine (Admin Control)")
                        st.markdown(
                            '<div style="background-color: #eef6ff; border-left: 5px solid #2563EB; padding: 10px; '
                            'border-radius: 4px; margin-bottom: 15px;">📌 यह टूल पूरे Master Database पर एक साथ दो नियम लागू करेगा:<br>'
                            '1️⃣ अगर किसी रो में <b>Subject</b> खाली है, तो उसमें <b>Degree</b> की वैल्यू भर दी जाएगी।<br>'
                            '2️⃣ अगर <b>Degree</b> में ब्रैकेट में कोई नंबर लिखा है (जैसे <i>"B.Sc. (3)"</i>), तो वह नंबर '
                            '<b>Duration</b> कॉलम में चला जाएगा और Degree से ब्रैकेट व उसके अंदर का पूरा हिस्सा हटा दिया जाएगा '
                            '(जैसे Degree सिर्फ <i>"B.Sc."</i> रह जाएगा)।</div>',
                            unsafe_allow_html=True
                        )

                        if st.session_state.admin_lock_state:
                            st.warning("🔒 **यह टूल अभी लॉक है:** इसे चलाने के लिए ऊपर जाकर पहले '🔓 लिस्ट अनलॉक करें (Editable)' बटन दबाएं।")
                        else:
                            import re as _re_fix_engine

                            # 🔍 पहले प्रीव्यू बनाएं ताकि Apply करने से पहले पता चल सके कितनी और कौन सी रो बदलेंगी
                            fix_preview_rows = []
                            for _idx, _row in live_db.iterrows():
                                _degree_val = str(_row.get("Degree", "")).strip()
                                _subject_val = str(_row.get("Subject", "")).strip()
                                _duration_val = str(_row.get("Duration", "")).strip()

                                _bracket_match = _re_fix_engine.search(r'\(([^)]*)\)', _degree_val)
                                _new_degree = _degree_val
                                _new_duration = _duration_val
                                _changed = False

                                if _bracket_match:
                                    _num_match = _re_fix_engine.search(r'\d+', _bracket_match.group(1))
                                    if _num_match:
                                        _new_duration = _num_match.group()
                                    _new_degree = _re_fix_engine.sub(r'\s*\([^)]*\)', '', _degree_val).strip()
                                    if _new_degree != _degree_val or _new_duration != _duration_val:
                                        _changed = True

                                _new_subject = _subject_val
                                if _subject_val == "" or _subject_val.lower() == "nan":
                                    if _new_degree != "":
                                        _new_subject = _new_degree
                                        _changed = True

                                if _changed:
                                    fix_preview_rows.append({
                                        "Row Index": _idx,
                                        "Student Name": _row.get("Student Name", ""),
                                        "Subject (पहले)": _subject_val, "Subject (बाद में)": _new_subject,
                                        "Degree (पहले)": _degree_val, "Degree (बाद में)": _new_degree,
                                        "Duration (पहले)": _duration_val, "Duration (बाद में)": _new_duration
                                    })

                            if not fix_preview_rows:
                                st.success("✅ पूरे डेटाबेस में इस नियम के अनुसार बदलने के लिए कोई भी रो नहीं मिली — सब कुछ पहले से ठीक है।")
                            else:
                                st.info(f"🔍 कुल **{len(fix_preview_rows)}** रो में बदलाव लागू होगा। नीचे प्रीव्यू देखें:")
                                st.dataframe(pd.DataFrame(fix_preview_rows), use_container_width=True, hide_index=True)

                                confirm_autofix = st.checkbox(
                                    "हाँ, मैंने प्रीव्यू देख लिया है और मैं यह बदलाव पूरे Master Database पर Apply करना चाहता हूँ।",
                                    key="p15_confirm_subject_degree_autofix"
                                )
                                if st.button(
                                    "🔧 Apply Subject/Degree/Duration Auto-Fix Now",
                                    type="primary", use_container_width=True,
                                    disabled=not confirm_autofix,
                                    key="p15_apply_subject_degree_autofix_btn"
                                ):
                                    try:
                                        _fix_counter = 0
                                        for _prow in fix_preview_rows:
                                            _ridx = _prow["Row Index"]
                                            live_db.at[_ridx, "Subject"] = _prow["Subject (बाद में)"]
                                            live_db.at[_ridx, "Degree"] = _prow["Degree (बाद में)"]
                                            live_db.at[_ridx, "Duration"] = _prow["Duration (बाद में)"]
                                            _fix_counter += 1

                                        save_live_data(live_db)
                                        st.success(f"🎉 सफलता! कुल {_fix_counter} छात्र रिकॉर्ड्स पर Subject/Degree/Duration Auto-Fix लागू करके Master Database में सुरक्षित सेव कर दिया गया है!")
                                        st.balloons()
                                        st.rerun()
                                    except Exception as _autofix_err:
                                        st.error(f"Auto-Fix Apply करते समय तकनीकी समस्या आई: {_autofix_err}")
