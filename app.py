...
DEFAULT_PRE_LOGIN_CONFIG = {
    ...
    # 🎨 ग्लोबल ऐप थीम सेटिंग्स (सभी 15 पैनल्स पर लागू होगी)
...
# 🎨 रेडी-मेड थीम प्रीसेट्स (P12 → Part 3 में एडमिन एक क्लिक में चुन सकता है)
THEME_PRESETS = {
    "Ocean Blue":     {"primary_color": "#2563EB", "sidebar_bg_color": "#0F172A", "app_bg_color": "#F4F7FB"},
    "Royal Purple":   {"primary_color": "#7C3AED", "sidebar_bg_color": "#1E1B2E", "app_bg_color": "#F6F4FB"},
    "Emerald Green":  {"primary_color": "#059669", "sidebar_bg_color": "#0B2A20", "app_bg_color": "#F2FAF6"},
    "Sunset Orange":  {"primary_color": "#EA580C", "sidebar_bg_color": "#2B1608", "app_bg_color": "#FFF7F2"},
    "Crimson Red":    {"primary_color": "#DC2626", "sidebar_bg_color": "#2B0A0A", "app_bg_color": "#FDF4F4"},
    "Midnight Dark":  {"primary_color": "#6366F1", "sidebar_bg_color": "#020617", "app_bg_color": "#EEF0FA"},
    "Rose Gold":      {"primary_color": "#E11D74", "sidebar_bg_color": "#2B0E1D", "app_bg_color": "#FEF3F8"},
    "Custom":         {}
}
...
# ==========================================================
# 🎨 स्टेप 4: डायनेमिक सीएसएस (CSS) रेंडरिंग इंजन — ग्लोबल मास्टर थीम इंजन
#            (यह एक जगह से बदलने पर सभी 15 पैनल्स पर लाइव अप्लाई होता है)
# ==========================================================
...
primary_color   = st.session_state.pre_login_config.get("primary_color", "#2563EB")
sidebar_color   = st.session_state.pre_login_config.get("sidebar_bg_color", "#0F172A")
app_bg_color    = st.session_state.pre_login_config.get("app_bg_color", "#F4F7FB")
font_choice     = st.session_state.pre_login_config.get("font_choice", "Poppins")
...
st.markdown(f"""
    ...
    <link href="https://fonts.googleapis.com/css2?family={font_choice.replace(' ', '+')}:wght@400;500;600;700;800&display=swap" rel="stylesheet">
    <style>
    :root {{
    ...
    html, body, [class*="css"] {{
        font-family: '{font_choice}', 'Segoe UI', sans-serif !important;
    }}

    /* 🌈 मुख्य ऐप बैकग्राउंड */
    [data-testid="stAppViewContainer"] {{
        background: linear-gradient(180deg, var(--app-bg) 0%, #ffffff 320px) !important;
    }}
    ...
    /* साइडबार रेडियो को नेविगेशन पिल्स की तरह दिखाना */
    ...
    /* 🔤 हेडिंग्स — बाईं तरफ रंगीन एक्सेंट बार */
    h1, h2, h3 {{ font-weight: 700 !important; color: #111827 !important; letter-spacing: -0.2px; }}
    ...
    .main h3 {{ color: var(--primary) !important; }}

    /* 🔘 बटन — ग्रेडिएंट, राउंडेड, होवर लिफ्ट इफ़ेक्ट */
    ...
    .stButton > button[kind="primary"]:hover, .stFormSubmitButton > button[kind="primary"]:hover {{
        ...
        box-shadow: 0 8px 18px var(--primary-glow) !important;
    }}
    ...
    /* 📑 टैब्स — पिल स्टाइल एक्टिव अंडरलाइन */
    ...
    /* 🗂️ इनपुट फ़ील्ड्स — राउंडेड + फोकस ग्लो */
    ...
    /* 🧾 फॉर्म / एक्सपैंडर / कंटेनर — सॉफ्ट कार्ड */
    [data-testid="stForm"] {{
        background: #ffffff;
    ...
    /* 🔔 अलर्ट बॉक्स — सॉफ्ट राउंडेड शैडो */
    ...
    if not active_tabs_names:
        ...
        # ----------------------------------------------------------------------
            ...
            if role == "full_admin":
                ...
                # --- Part 3: 🎨 Global App Theme Designer (सभी 15 पैनल्स पर लाइव अप्लाई) ---
                st.subheader("🎨 Part 3: Global App Theme Designer (सभी 15 पैनल्स के लिए)")
                st.markdown(
                    '<div style="background-color:#eef2ff; border-left:5px solid #6366F1; padding:12px; '
                    'border-radius:4px; margin-bottom:16px;">🖌️ यहाँ से पूरे सिस्टम (साइडबार, बटन्स, हेडिंग्स, '
                    'टेबल्स — सभी 15 पैनल्स) का रंग और फ़ॉन्ट एक साथ बदल सकते हैं।</div>',
                    unsafe_allow_html=True
                )

                current_preset = st.session_state.pre_login_config.get("theme_preset", "Ocean Blue")
                preset_options = list(THEME_PRESETS.keys())
                chosen_preset = st.selectbox(
                    "🎁 रेडी-मेड थीम प्रीसेट चुनें (या नीचे खुद Custom रंग सेट करें):",
                    options=preset_options,
                    index=preset_options.index(current_preset) if current_preset in preset_options else 0,
                    key="p12_theme_preset_selector"
                )

                # प्रीसेट चुनने पर डिफ़ॉल्ट रंग भरें, "Custom" पर मौजूदा सेव्ड रंग दिखाएँ
                if chosen_preset != "Custom" and THEME_PRESETS.get(chosen_preset):
                    _preset_vals = THEME_PRESETS[chosen_preset]
                    default_primary = _preset_vals["primary_color"]
                    default_sidebar = _preset_vals["sidebar_bg_color"]
                    default_appbg = _preset_vals["app_bg_color"]
                else:
                    default_primary = st.session_state.pre_login_config.get("primary_color", "#2563EB")
                    default_sidebar = st.session_state.pre_login_config.get("sidebar_bg_color", "#0F172A")
                    default_appbg = st.session_state.pre_login_config.get("app_bg_color", "#F4F7FB")

                with st.form(key="p12_global_theme_form"):
                    col_th1, col_th2, col_th3 = st.columns(3)
                    with col_th1:
                        theme_primary = st.color_picker("🔵 प्राइमरी रंग (बटन/हेडिंग/एक्सेंट)", value=default_primary)
                    with col_th2:
                        theme_sidebar = st.color_picker("🌑 साइडबार बैकग्राउंड रंग", value=default_sidebar)
                    with col_th3:
                        theme_appbg = st.color_picker("⬜ ऐप बैकग्राउंड रंग", value=default_appbg)

                    font_options = ["Poppins", "Inter", "Roboto", "Nunito", "Lato", "Montserrat"]
                    current_font = st.session_state.pre_login_config.get("font_choice", "Poppins")
                    theme_font = st.selectbox(
                        "🔤 फ़ॉन्ट स्टाइल चुनें:",
                        options=font_options,
                        index=font_options.index(current_font) if current_font in font_options else 0
                    )

                    apply_theme_btn = st.form_submit_button(
                        "🚀 Apply Theme to All 15 Panels", type="primary", use_container_width=True
                    )

                    if apply_theme_btn:
                        updated_theme_config = dict(st.session_state.pre_login_config)
                        updated_theme_config.update({
                            "theme_preset": chosen_preset,
                            "primary_color": theme_primary,
                            "sidebar_bg_color": theme_sidebar,
                            "app_bg_color": theme_appbg,
                            "font_choice": theme_font
                        })
                        st.session_state.pre_login_config = updated_theme_config
                        save_pre_login_config(updated_theme_config)
                        st.success("🎉 नई थीम पूरे सिस्टम के सभी 15 पैनल्स पर लाइव अप्लाई हो गई है!")
                        st.balloons()
                        st.rerun()
            else:
